...
void LCD::drawBitmapToScreen(int8_t X,int8_t Y,int8_t Width,int8_t Height,const uint8_t *Bitmap)
{
};
void LCD::drawBitmapToScreenPGM(int8_t X,int8_t Y,int8_t Width,int8_t Height,const uint8_t *Bitmap)
{
  int8_t i,j,byteWidth = (Width + 7) / 8;
  int8_t data;
  
  for (j = floor(Y / 8); j < floor(Y / 8) + ceil(Height / 8); j++)
  {
    for (i = floor(X / 8); i < floor(X / 8) + ceil(Width / 8); i++)
    {
      XY(i,j);
      WriteData(pgm_read_byte(Bitmap + j * Width + i));
    };
  };
};
